...
namespace solana {
    namespace rpc {
        ...
        /// 1. Build requests
        ///
        json Solana::getAccountInfoRequest(const std::string &account, const std::string &encoding) {
            const json params = {
                    account,
                    {{"encoding", encoding}}};
        ...
        json Solana::sendTransactionRequest(const std::string &transaction, const std::string &encoding,
                                            bool skipPreflight,
                                            const std::string &preflightCommitment) {
            const json params = {
                    transaction,
                    {{"encoding", encoding}, {"skipPreflight", skipPreflight},
        ...
        ///
        PublicKey Solana::getRecentBlockhash(const std::string &commitment) {
        ...
        json Solana::getSignatureStatuses(const std::vector <std::string> &signatures,
        ...
        std::string Solana::signAndSendTransaction(const Keypair &keypair, const CompiledTransaction &tx,
                                                   bool skipPreflight,
                                                   const std::string &preflightCommitment) {
