...
void on_open(ws_client *c, websocketpp::connection_hdl hdl) {
  ...
  c->send(hdl, solana::rpc::accountSubscribeRequest(account).dump(),
  ...
  if (events->header.seqNum > lastSeqNum) {
    for (int offset = seqNumDiff; offset > 0; --offset) {
      ...
      case mango_v3::EventType::Liquidate: {
        ...
        spdlog::info("LIQ prc: {} qty: {}", liq.price.toDouble(), liq.quantity);
        break;
      }
      }

      const uint64_t lag = std::chrono::duration_cast
...
int main() {
  ...
  try {
    ...
    c.set_open_handler(
        websocketpp::lib::bind(&on_open, &c,
            websocketpp::lib::placeholders::_1));
    c.set_message_handler(
        websocketpp::lib::bind(&on_message, &c,
            websocketpp::lib::placeholders::_1,
