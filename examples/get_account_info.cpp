...
int main()
{
  ...
  if (curl)
  {
    ...
    const json req = solana::rpc::getAccountInfoRequest(pubKey);
    ...
    if (!err)
    {
      ...
      for (int i = 0; i < mango_v3::MAX_TOKENS; ++i)
      {
        ...
        const auto mintPk = sol::Base58::b58encode(std::string((char *)token.mint.data, 32));

        if (mintPk == std::string("11111111111111111111111111111111"))
          continue;

        const auto rootBankPk = sol::Base58::b58encode(std::string((char *)token.rootBank.data, 32));
      ...
      for (int i = 0; i < mango_v3::MAX_PAIRS; ++i)
      {
        ...
        const auto marketPk = sol::Base58::b58encode(std::string((char *)market.spotMarket.data, 32));
      ...
      for (int i = 0; i < mango_v3::MAX_PAIRS; ++i)
      {
        ...
        const auto marketPk = sol::Base58::b58encode(std::string((char *)market.perpMarket.data, 32));
      ...
      for (int i = 0; i < mango_v3::MAX_PAIRS; ++i)
      {
        const auto oraclePk = sol::Base58::b58encode(std::string((char *)group->oracles[i].data, 32));
      ...
      std::cout << "signerKey: " << sol::Base58::b58encode(std::string((char *)group->signerKey.data, 32)) << std::endl;
      std::cout << "admin: " << sol::Base58::b58encode(std::string((char *)group->admin.data, 32)) << std::endl;
      std::cout << "dexProgramId: " << sol::Base58::b58encode(std::string((char *)group->dexProgramId.data, 32)) << std::endl;
      std::cout << "mangoCache: " << sol::Base58::b58encode(std::string((char *)group->mangoCache.data, 32)) << std::endl;
      std::cout << "validInterval: " << group->validInterval << std::endl;
      std::cout << "insuranceVault: " << sol::Base58::b58encode(std::string((char *)group->insuranceVault.data, 32)) << std::endl;
      std::cout << "srmVault: " << sol::Base58::b58encode(std::string((char *)group->srmVault.data, 32)) << std::endl;
      std::cout << "msrmVault: " << sol::Base58::b58encode(std::string((char *)group->msrmVault.data, 32)) << std::endl;
      std::cout << "feesVault: " << sol::Base58::b58encode(std::string((char *)group->feesVault.data, 32)) << std::endl;
