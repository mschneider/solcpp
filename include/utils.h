...
splitOpenOrders(const serum_v3::OpenOrders* openOrders){
  const auto quoteFree = i80f48((openOrders->quoteTokenFree
                                 + openOrders->referrerRebatesAccrued));
  const auto quoteLocked = i80f48((openOrders->quoteTokenTotal
                                   - openOrders->quoteTokenFree));
  const auto baseFree = i80f48((openOrders->baseTokenFree));
  const auto baseLocked = i80f48((openOrders->baseTokenTotal
...
i80f48 getUnsettledFunding(const PerpAccountInfo* accountInfo, const PerpMarketCache* perpMarketCache){
  if(accountInfo->basePosition < 0) {
    return i80f48(accountInfo->basePosition) * (perpMarketCache->short_funding - accountInfo->shortSettledFunding);
  } else {
    return i80f48(accountInfo->basePosition) * (perpMarketCache->long_funding - accountInfo->longSettledFunding);
  }
}
...
i80f48 getAssetVal(PerpAccountInfo* perpAccountInfo, PerpMarketInfo* perpMarketInfo,
                   i80f48 price, i80f48 shortFunding, i80f48 longFunding){
  auto assetsVal = i80f48(0ULL);
  if (perpAccountInfo->basePosition > 0){
    assetsVal += (i80f48(perpAccountInfo->basePosition * perpMarketInfo->baseLotSize) * price);
  }
  auto realQuotePosition = perpAccountInfo->quotePosition;
  if(perpAccountInfo->basePosition > 0){
    realQuotePosition = perpAccountInfo->quotePosition - ((longFunding-perpAccountInfo->longSettledFunding) * i80f48(perpAccountInfo->basePosition));
  } else if(perpAccountInfo->basePosition < 0) {
    realQuotePosition = perpAccountInfo->quotePosition - ((shortFunding-perpAccountInfo->shortSettledFunding) * i80f48(perpAccountInfo->basePosition));
  }
...
i80f48 getLiabsVal(PerpAccountInfo* perpAccountInfo, PerpMarketInfo* perpMarketInfo, i80f48 price, i80f48 shortFunding, i80f48 longFunding){
  auto liabsVal = i80f48(0ULL);
  if(perpAccountInfo->basePosition < 0){
    liabsVal += (i80f48(perpAccountInfo->basePosition * perpMarketInfo->baseLotSize) * price);
  }
  auto realQuotePosition = perpAccountInfo->quotePosition;
  if(perpAccountInfo->basePosition > 0){
    realQuotePosition = perpAccountInfo->quotePosition - ((longFunding - perpAccountInfo->longSettledFunding) * i80f48(perpAccountInfo->basePosition));
